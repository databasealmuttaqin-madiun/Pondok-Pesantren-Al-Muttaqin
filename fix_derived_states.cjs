...
// Remove nominalNumber and proyeksiKas
const derivedStatesRegex = /const nominalNumber = [\s\S]*?\}, \[currentKasSelectedKantin, nominalNumber, jenis\]\);/;
const newDerivedStates = `
  const uangMasukNum = parseInt(uangMasukStr.replace(/\\D/g, "") || "0", 10);
  const uangKeluarNum = parseInt(uangKeluarStr.replace(/\\D/g, "") || "0", 10);
  const proyeksiKas = currentKasSelectedKantin + uangMasukNum - uangKeluarNum;
`;
code = code.replace(derivedStatesRegex, newDerivedStates);

// Right Column ringkasan Kas Terkini & Proyeksi
const rightColumnRegex = /\{\/\* Ringkasan & Kalkulator Kas \*\/\}\s*<div className="space-y-4">[\s\S]*?\{\/\* Riwayat Transaksi Hari Ini \*\/\}/;
const newRightColumn = `{/* Ringkasan & Kalkulator Kas */}
