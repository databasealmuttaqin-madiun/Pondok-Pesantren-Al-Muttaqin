...
export default function PerizinanPanel({
  ...
  return (
    <div className="space-y-6">
      ...
      {connectionError && (
        <div className="bg-amber-50 border-2 border-amber-300 rounded-2xl p-4 text-amber-900 space-y-2 animate-fade-in" id="status-siswa-connection-alert">
          <div className="flex items-start gap-3">
            <span className="p-1.5 bg-amber-100 rounded-xl text-amber-700 font-bold shrink-0 text-base">⚠️</span>
            <div>
              <h4 className="font-extrabold text-amber-950 text-xs">
                Mode Penyimpanan Lokal Sementara Active
              </h4>
              <p className="text-[11px] leading-relaxed text-amber-800 mt-0.5">
                Sistem saat ini beralih ke Mode Penyimpanan Lokal Sementara (Offline Backup) agar Anda tetap dapat mengedit status perizinan.
              </p>
            </div>
          </div>
        </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                ...
                >
                  ...
                  <span className={`text-[10px] mt-2 block font-medium ${selectedKamar === "Belum Set" ? "text-sky-600 font-bold" : "text-slate-400"}`}>
                    👥 {students.filter(s => !s.kamar || s.kamar === "—").length} Siswa
                  </span>
                </button>
              </div>
            </div>

            {/* STEP 2: CHOOSE STUDENT / PILIH NAMA SISWA (Only available after Step 1) */}
            {selectedKamar && (
              <div className="space-y-2 animate-fade-in">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  <label className="text-xs font-black text-slate-500 uppercase tracking-wider flex items-center gap-1.5 select-none">
                    ...
                    PILIH NAMA SISWA ({filteredStudents.length} tersedia)
                  </label>
                ...
                {filteredStudents.length === 0 ? (
                  <div className="text-center py-6 bg-slate-50 border border-dashed border-slate-200 rounded-2xl">
                    <p className="text-xs text-slate-400 font-medium">Tidak ada siswa yang cocok atau terdaftar di kamar ini.</p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-48 overflow-y-auto pr-1">
                    {filteredStudents.map((student, idx) => {
                      ...
                      return (
                        <button
                          key={`perizinan-st-${student.id || student.nik || idx}-${idx}`}
                          type="button"
                          onClick={() => {
            ...
            {isLoadingHistory ? (
              ...
              <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
                {statusHistory.map((row, idx) => {
                  ...
                  return (
                    <div 
                      key={`perizinan-row-${row.id || row.nama || idx}-${idx}`} 
